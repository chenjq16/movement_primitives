...
from movement_primitives.dmp import DMP, DMPWithFinalVelocity
...
midpoint = (goal_y_right + start_y_left) / 2
...
X, Xd, Xdd = generate_minimum_jerk(goal_y_right, start_y_left, execution_time=20, dt=1)

T = np.arange(0, data_len_right+data_len_left+2, 1)
Y = np.concatenate((Y_right, Y_left), axis=0)
...
for _ in range(5):
    ...
    midpoint_random = midpoint + offset_random

    dmp_right.configure(start_y=start_y_random_right, goal_y=midpoint_random)
    T_right, Y_right = dmp_right.open_loop()
    ...
    V_right = np.zeros((data_len_right+1, 3))
    V_right[:,0] = np.gradient(Y_right[:,0]) / dmp_right.dt_
    V_right[:,1] = np.gradient(Y_right[:,1]) / dmp_right.dt_
    V_right[:,2] = np.gradient(Y_right[:,2]) / dmp_right.dt_
    V_final_right = V_right[-1, :]
    A_right = np.zeros((data_len_right+1, 3))
    A_right[:,0] = np.gradient(V_right[:,0]) / dmp_right.dt_
    A_right[:,1] = np.gradient(V_right[:,1]) / dmp_right.dt_
    A_right[:,2] = np.gradient(V_right[:,2]) / dmp_right.dt_
    A_final_right = A_right[-1, :]
    print(V_final_right)

    dmp_left.configure(start_y=midpoint_random, start_yd=V_final_right, start_ydd=A_final_right, goal_y=goal_y_random_left)
    T_left, Y_left = dmp_left.open_loop()
    ax.plot(Y_left[:, 0], Y_left[:, 1], Y_left[:, 2], c="orange")
    ax.scatter(midpoint_random[0], midpoint_random[1], midpoint_random[2], c="orange")
