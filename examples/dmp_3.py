...
dmp_right = DMP(n_dims=y_demo_right.shape[1], execution_time=execution_time_right, dt=1)
dmp_right.imitate(T_right, y_demo_right)

dmp_left = DMP(n_dims=y_demo_left.shape[1], execution_time=execution_time_left, dt=1)
...
for _ in range(3):
    start_y_random_right = start_y_right + 50 * _
    goal_y_random_right = goal_y_right + 40 * _

    start_y_random_left = start_y_left + 30 * _
    goal_y_random_left = goal_y_left + 20 * _
    ...
    dmp_left.configure(start_y=midpoint_random, goal_y=goal_y_random_left)
    T_left, Y_left = dmp_left.open_loop()
    ax.plot(Y_left[:, 0], Y_left[:, 1], Y_left[:, 2], c="orange")
    ax.scatter(midpoint_random[0], midpoint_random[1], midpoint_random[2], c="orange")

    T = np.arange(0, data_len_right+data_len_left+1, 1)
    Y = np.concatenate((Y_right, Y_left[1:,:]), axis=0)
